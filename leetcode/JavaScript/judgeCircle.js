...
var judgeCircle = function(moves) {
    var xDist = 0;
    var yDist = 0;
    for(var direction in moves) {
        if(moves[direction] == "U") { yDist++; }
        else if(moves[direction] == "D") { yDist--; }
        else if(moves[direction] == "L") { xDist++; }
        else if(moves[direction] == "R") { xDist--; }
    }
    return (xDist == 0 && yDist == 0) ? true : false;
};
